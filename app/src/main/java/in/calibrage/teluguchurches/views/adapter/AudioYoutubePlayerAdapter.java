...
public class AudioYoutubePlayerAdapter  extends RecyclerView.Adapter<AudioYoutubePlayerAdapter.MyHolder> implements YouTubeThumbnailView.OnInitializedListener {
    ...
    public AudioYoutubePlayerAdapter(Context context, GetPostByCategoryIdResponseModel dataList, ArrayList<String> linkArray) {
     ...
     * */
    @Override
    public AudioYoutubePlayerAdapter.MyHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_layout, null);
        AudioYoutubePlayerAdapter.MyHolder mh = new AudioYoutubePlayerAdapter.MyHolder(v);
        return mh;
    }
     ...
     * */
    @Override
    public void onBindViewHolder(AudioYoutubePlayerAdapter.MyHolder holder, final int position) {
        holder.textView_title.setText(dataList.getResult().getAudios().get(position).getTitle());
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("link", linkArray.get(position))
                        .putExtra("url", dataList.getResult().getAudios().get(position).getEmbededUrl())
                        .putExtra("postTypeId", dataList.getResult().getAudios().get(position).getId())
                        .putExtra("likeCount", dataList.getResult().getAudios().get(position).getLikeCount())
                        .putExtra("disLikeCount", dataList.getResult().getAudios().get(position).getDisLikeCount());
                postId = dataList.getResult().getAudios().get(position).getId();
                link = linkArray.get(position);
                url = dataList.getResult().getAudios().get(position).getEmbededUrl();
    ...
    //after successfull initialization of loder
    @Override
